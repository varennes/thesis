...
% \red{[Paragraph on the notion that chemotaxis is constrained intrinsically (cell-type specific properties) and extrinsically (environment). Here we focus on the environmental effects. Do different environmental perturbations have similar effects on these metrics?]}
...
We use the Cellular Potts Model (CPM) to simulate cell chemotaxis in a similar fashion as that prescribed in our previous paper \red{[CITE BIOPHYS]}. In the CPM cell's move on a lattice and are comprised of a finite number of lattice sites. Cell movement occurs through the stochastic addition and removal of lattice sites to the cell.
